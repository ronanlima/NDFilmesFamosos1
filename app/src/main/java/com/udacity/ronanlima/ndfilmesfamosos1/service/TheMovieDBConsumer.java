...
public class TheMovieDBConsumer {
    ...
    public static void getTopRatedMovies(final ListenerResultSearchTMDB listener) {
        retrofit.getRetrofit().create(TMDBInterface.class).getMostPopularMovies(BuildConfig.API_KEY, 1/*TODO receber page por parâmetro*/).enqueue(new Callback<JsonObject>() {
            @Override
